...
|---|---|---|---|
| **PR 1** | プロジェクト基盤の構築 | ・ディレクトリ構造の作成<br>・`requirements/` 分割ファイルの配置<br>・`tox.ini` (lint, unittest) の作成<br>・`check_filenames.py` の作成<br>・`.github/workflows/ci.yml` (GitHub Actions CI)<br>・`.gitignore`, `.env.example`, `README.md` | `tox -e lint` 疎通確認、GitHub Actions CI の動作確認 |
