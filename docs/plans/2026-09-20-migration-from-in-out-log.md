...
- [x] PR 1: プロジェクト基盤の構築
- [x] PR 2: 設定管理 & DBアクセス層
- [x] PR 3: 外部IoT連携クライアント層
- [x] PR 4: コアビジネスロジック層
- [x] PR 5: Webサーバー層 (Blueprint)
- [ ] PR 6: 常駐バッチワーカー & CLI (非同期化計画へ移行のため中止)
- [ ] PR 7: コンテナ化 & デプロイ構成 (非同期化計画へ移行のため中止)
- [ ] 本番デプロイ & 切り替え完了 (非同期化計画へ移行のため中止)

---

## 5. 追記: 非同期処理化 (asyncio対応) 計画の挟み込み (2026-09-21)

PR 5 のマージ完了タイミングにおいて、バッチ処理および Web サーバーのレスポンス性能・並列処理効率向上のため、`asyncio` による非同期処理化案件を割り込ませる方針となりました。
詳細な実行計画は [docs/plans/2026-09-21-async-execution-plan.md](2026-09-21-async-execution-plan.md) を参照してください。
